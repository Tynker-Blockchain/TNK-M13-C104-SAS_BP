...
def createAccount():
    ...
    if request.method == "POST":
        new_account_name = request.form.get("newAccountName")

        if new_account_name:
            account = Account(username, accountName=new_account_name)
        else:
            account = Account(username, accountName=None)
...
# Create route /deleteAccountName

# Define deleteAccountName() function
    ...
    # Check is type of account variable is dict
        ...
        #Access the address of account using key


        #Else, access the address parameter of account object


    #Call the deleteAccountName() function and pass the address

    # Select the account with first account from allAccounts list and store it in account variable   

    # After deleting the account, you can redirect the user to a different page or the homepage.
