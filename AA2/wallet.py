...
class Account():
    def __init__(self, username, accountName):
        self.account = web3.eth.account.create()
        self.address = self.account.address
        self.privateKey = self.account.key.hex()
        self.addToDB(self.address, self.privateKey, username, accountName)
        
    def addToDB(self, address, privateKey, username, accountName):
    ...
    def checkConnection(self):
        if web3.is_connected():
           return True
            ...
            return False
    #Create deleteAccountName() function to delete the account name.

        # Initialize a reference to your Firebase database

        # Get the account node by its address

        # Update the account node to remove the account name
